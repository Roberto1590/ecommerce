...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 page-titles">
        <div class="welcome-text">
            <h4 class="fw-bold py-3 mb-4">
                <span class="text-muted fw-light">Comercio /</span> Categorias
            </h4>
        </div>
        <div>
                        ...
                        <tr data-id="{{ $categoria->id }}">
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-3">
                                    ...
                                    <form id="formDelete" action="{{ route('auth.delete.dashboard.comercio.categorias') }}"
                                        method="post">
                                        @method('delete')
                                        @csrf
                                ...
                                <tr data-id="{{ $categoria->id }}">
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center">
                                            <form id="formrestore"
                                                action="{{ route('auth.restore.dashboard.comercio.categorias') }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                <input type="hidden" value="{{ $categoria->id }}" name="id">
    ...
    <div class="modal fade" id="newModal">
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalCenterTitle">Nova Categoria</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="formnew" action="{{ route('auth.post.dashboard.comercio.categorias') }}" method="post">
                        @csrf
                        ...
                        <div class="form-group mb-3">
                            <label for="nome">Categoria Pai</label>
                            <select name="categoria_pai_id" id="categoria_pai_id" class="form-control">
                                @foreach ($catPais as $cat)
    ...
    <div class="modal fade" id="EditModal">
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('auth.put.dashboard.comercio.categorias') }}" method="post">
                        @method('put')
                        @csrf
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="categoria_pai_id" id="categoria_pai_id" class="form-control">
                                @foreach ($catPais as $cat)
                                    <option class="opt_categoria" value="{{ $cat->id }}">
                                        {{ $cat->nome }}
                                    </option>
                                @endforeach
                            </select>
...
@push('script')
    ...
    <script>
        ...
        $('.btnEdit').on('click', function() {
            ...
            // URL
            let urlCompleta = Url + '/dashboard/comercio/categorias/' + id + '/get';

            $('.opt_categoria').attr('selected', false);
            // AJAX
            $.get(urlCompleta, function(data) {
                $('#idEdit').val(data.id);
                $('.opt_categoria[value="' + data.categoria_pai.id + '"]').attr('selected', true);
        ...
        $(".btnrestore").on("click", function() {
            ...
            const routeRestore = Url + "{{ route('auth.restore.dashboard.comercio.categorias') }}?id=" + id;
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    formRestore.action = routeRestore; // Passando URL para action do form
                    formRestore.submit();
                }
            })
        });
        ...
        $(".btnremove").on("click", function() {
            ...
            const routeDelete = Url + "{{ route('auth.delete.dashboard.comercio.categorias') }}?id=" + id;
            Swal.fire({
